...
- **v2.4.0** — แก้ปัญหาข้อมูลหาย + auto-logout · ใช้ **safe MERGE sync** แทน delete-then-insert (ไม่ทำลายข้อมูล) · **Auto-sync** ทุกครั้งที่มีการเปลี่ยนแปลง (debounced 2s) + ทุก 5 นาที + ตอนกลับมาที่แท็บ · Session persists (autoRefreshToken + localStorage) · Sync dot indicator ที่ header (เขียว=ซิงค์แล้ว, เหลือง=กำลังซิงค์, แดง=ผิดพลาด, เทา=ออฟไลน์) · ต้อง run [`calpro-database-v2.4-migration.sql`](calpro-database-v2.4-migration.sql) ใน Supabase
