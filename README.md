...
- **v2.36.9** *(เว็บ · BETA · silent)* — 🧱 **Epic A เก็บงานย่อย (จบ Epic)** — เพิ่ม helper `cloudState()` (รวมการเช็ก "ล็อกอินจริง/ไม่ระบุตัวตน" ไว้จุดเดียว · Epic B จะใช้) + เพิ่มชุดทดสอบเป็น **20 ข้อ** (เพิ่มเทสต์ cloudState + เช็กเวอร์ชัน SW cache ตรงกับ APP_VERSION กันเลขเวอร์ชันหลุดกัน) · ไม่กระทบการใช้งาน · อัปเดตเงียบ
- **v2.36.8** *(เว็บ · BETA · silent)* — 🧱 **Epic A (slice 2b): กู้ข้อมูลอัตโนมัติตอนบูตจาก IndexedDB** — ตอนเปิดแอป ถ้า localStorage ถูกตัดทอน (เพราะพื้นที่เคยเต็ม) หรือว่างเปล่า จะ **เทียบ+รวมกับ backup ใน IndexedDB (ก้อนที่ครบที่สุด) แบบ "วันไหนมีมากกว่าใช้อันนั้น"** → ข้อมูลกลับมาครบเองโดยไม่ต้องพึ่ง localStorage ที่จำกัด · **ปิดช่องข้อมูลหายจากพื้นที่เต็มแบบถาวร** · ทดสอบ: ตัดเหลือ 5 วัน→รีโหลด→กลับมา 31 วันครบ, self-test 18/18 ผ่าน · อัปเดตเงียบ
- **v2.36.7** *(เว็บ · BETA · silent)* — 🧱 **Epic A (slice 2a): ย้ายข้อมูลสำรองไป IndexedDB** — ตัวสำรองในเครื่องย้ายจาก localStorage (เพดาน ~5MB) ไป **IndexedDB (โควต้าใหญ่มาก)** → สำรองได้ครบทุกวันแม้ประวัติยาว และ **ไม่แย่งพื้นที่กับข้อมูลหลักอีกต่อไป** (ต้นเหตุข้อมูลหายตอนเต็ม) · มีการกู้คืนอัตโนมัติแบบ async (สำรอง LS เดิมยังใช้กู้ได้ช่วงเปลี่ยนผ่าน แล้วลบทิ้งเพื่อคืนที่ว่าง) · ทดสอบแล้ว: ลบคีย์หลัก→รีโหลด→กู้คืนครบ 31 วันจาก IDB, self-test 12/12 ผ่าน · อัปเดตเงียบ
- **v2.36.6** *(เว็บ · BETA · silent)* — 🧱 **Epic A เริ่ม: รากฐาน/ความเสถียร (slice 1)** — เพิ่ม **ชุดทดสอบในแอป** (Developer Mode → "Run self-tests") ตรวจฟังก์ชันสำคัญ 12 ข้อ (สตรีค/รูปโปรไฟล์/วันที่/กันข้อมูลหายตอนพื้นที่เต็ม) เป็นตาข่ายกันพังก่อนงานใหญ่ · รวม helper ซ้ำ (`isEn()`, `utcDay()`) เป็นจุดเดียว · ผลลัพธ์เหมือนเดิมทุกประการ · อัปเดตเงียบ
- **v2.36.5** *(เว็บ · BETA · silent)* — 🧹 **รีแฟกเตอร์ (คุณภาพโค้ด · พฤติกรรมเหมือนเดิม)** — รวมโค้ดวาดรูปโปรไฟล์ที่ซ้ำกัน (หน้าเพื่อน + ค้นหาเพื่อน) เป็นฟังก์ชันเดียว `avatarHtml()` · พิสูจน์แล้วว่าผลลัพธ์ HTML เหมือนเดิมทุก byte (รูปจริง/อีโมจิ/ตัวอักษร) · ไม่กระทบการใช้งาน · อัปเดตเงียบ
- **v2.36.4** *(เว็บ · BETA · silent)* — 🛡️ **กันข้อมูลหายตอนพื้นที่เต็ม (bugfix)** — เดิมถ้า localStorage เต็ม (ประวัติยาว + รูปมื้อที่เก็บแบบ inline + คีย์สำรอง) การบันทึกอาหารจะ **ล้มเหลวเงียบๆ** (เห็นในแอปแต่หายตอนรีโหลด) · ตอนนี้ถ้าเจอ QuotaExceededError จะ **สละคีย์สำรองอัตโนมัติเพื่อเคลียร์ที่ว่างแล้วบันทึกซ้ำ** → ข้อมูลหลักไม่หาย · `lsSave` คืนค่า true/false ให้ตรวจได้ · อัปเดตเงียบ
- **v2.36.3** *(เว็บ · BETA · silent)* — ⚡ **เพิ่มความเร็ว (perf)** — ย้ายการ "สำรองข้อมูลในเครื่อง" ออกจากจังหวะกดเพิ่มอาหาร/เปิดแอป → ทำตอนเครื่องว่าง (requestIdleCallback) แทน ไม่หน่วงตอนใช้งาน · เขียนถี่ขึ้นเป็นทุก 20 วิ + flush ก่อนปิดแอป (ข้อมูลไม่หาย) + ข้ามถ้าใหญ่เกินโควต้า · ผู้ใช้ที่มีประวัติยาวๆ (หลายพันรายการ) จะลื่นขึ้นชัดเจนบนมือถือ · อัปเดตเงียบ
- **v2.36.2** *(เว็บ · BETA · silent)* — 🎨 **วงแหวนไล่สีแบบ gradient (เย็น→อุ่น) + เก็บไอคอนเมนูซ้าย** — (1) วงแหวนแคลอรี่กลับมาเป็น **แถบไล่สีจริง** (ฟ้า→เขียว→เหลือง→ส้ม→แดง) ไล่จากโทนเย็นไปอุ่น · เกินเป้า = แดงทั้งวง · (2) เอา **ไอคอนดินสอ/แก้รูป** บนรูปโปรไฟล์ในเมนูซ้ายออก (ยังแตะที่รูปเพื่อเปลี่ยนได้เหมือนเดิม) · อัปเดตเงียบ
- **v2.36.1** *(เว็บ · BETA · silent)* — 🎨 **เก็บรายละเอียดหน้าแรก + เมนูซ้าย** — (1) เมนูซ้าย: รูปโปรไฟล์มีไอคอนเปลี่ยนรูปซ้อนกัน 2 อัน → เหลือ **กล้องอันเดียว** · (2) **ตัดแถบบาร์ใต้วงกลมแคลอรี่** ที่ไม่แสดงผลออก (ซ้ำกับวงแหวน) · (3) **วงแหวนแคลอรี่ไล่สีใหม่ตามสถานะ** — โทนเย็น→อุ่น (ฟ้า→เขียว→เหลือง→ส้ม→แดง) บอกชัดว่ายิ่งเข้าใกล้/เกินเป้ายิ่งแดง · อัปเดตเงียบ
- **v2.36.0** *(เว็บ · BETA · silent)* — 🧩 **แก้/เพิ่มหลายจุด** — (1) 👥 **แก้บั๊กหน้าเพื่อน** — เพิ่มเพื่อนแล้วไม่ขึ้นในรายการ (RLS own-only บล็อกการ join โปรไฟล์เพื่อน) → ดึงโปรไฟล์เพื่อนผ่าน RPC `get_public_profiles` แทน · (2) 🌐 **เลือกภาษาก่อนใช้งานครั้งแรก** — ผู้ใช้ใหม่จะเจอหน้าจอเลือกภาษา (ไทย/อังกฤษ/简体/繁體) ก่อนเข้า onboarding · (3) 📷 **กดรูปโปรไฟล์ในเมนูซ้าย = แก้รูปได้เลย** (เดิมเด้งไปหน้าโปรไฟล์) + มีไอคอนกล้องบอกชัด · (4) ☁️ **หน้า Cloud Sync โชว์รูปโปรไฟล์แทนไอคอนเมฆ** เมื่อล็อกอินแล้ว · (5) 📲 **ป๊อปอัปชวนติดตั้งแอป (PWA)** — Android กดติดตั้งได้เลย (beforeinstallprompt) · iOS แสดงวิธี "เพิ่มไปยังหน้าจอโฮม" · ข้ามอัตโนมัติถ้าติดตั้งแล้ว/เป็นแอป APK · อัปเดตเงียบ
- **v2.35.2** *(เว็บ · BETA)* — 🔥 **แก้สตรีคแสดงผลถูกต้อง** — ถ้ายังไม่บันทึกอาหารของวันนี้ สตรีคจะไม่รีเซ็ตเป็น 0 (เดิมพอข้ามวันแล้วยังไม่กินอะไร สตรีคหายทันที) → ตอนนี้นับวันต่อเนื่องถึงเมื่อวานให้ สตรีคยังอยู่จนกว่าจะหมดวันจริง · ใช้ทั้งหน้าโปรไฟล์/วิเคราะห์/เลเวล/การ์ดแชร์
- **v2.35.1** *(เว็บ · BETA)* — 🛟 **กันข้อมูลหาย (สำคัญ)** — (1) เพิ่ม **สำรองข้อมูลอัตโนมัติในเครื่อง** + **กู้คืนเอง** ทันทีถ้าคีย์ข้อมูลหลักหาย/เสีย (กันกรณีล้างแคชบางส่วน/ข้อมูลพัง) · (2) **แก้บั๊กซิงค์ร้ายแรง** — เดิมถ้าข้อมูลในเครื่องว่าง (เช่นเพิ่งล้างแคช) แล้วล็อกอินอยู่ การ push อาจไป **ลบข้อมูลบนคลาวด์ทิ้งด้วย** · ตอนนี้จะ **ไม่ลบข้อมูลบนคลาวด์จนกว่าจะดึง (pull) ข้อมูลมาสำเร็จก่อน** (ยืนยันว่าเครื่องมีข้อมูลครบ) ครอบคลุมทั้งอาหาร/ออกกำลังกาย/ของโปรด/อาหารที่สร้างเอง · *หมายเหตุ: การอัปเดตแอปเองไม่เคยลบข้อมูล — ข้อมูลเก็บใน localStorage ของเบราว์เซอร์*
- **v2.35.0** *(เว็บ · BETA)* — 🎓 **ทัวร์แนะนำการใช้งานแบบทีละขั้น** — เปิดแอปครั้งแรกตั้งแต่เวอร์ชันนี้ (ทั้งผู้ใช้ใหม่และเก่า) จะมีหน้าต่างสอนใช้งานเด้งขึ้นมาให้อัตโนมัติ **ครั้งเดียว** · มี **7 สไลด์** พร้อม **ภาพจำลองหน้าจอ** ของแต่ละหน้า + **ไฮไลต์จุดที่ต้องกดเป็นลำดับขั้น (①②③)** + คำอธิบายเข้าใจง่าย (วันนี้ · เพิ่มอาหาร · ถ่ายรูป AI · วิเคราะห์ · โปรไฟล์/เพื่อน · เมนู) · กด **ดูซ้ำได้ทุกเมื่อ** จากเมนูฝั่งซ้าย ("ทัวร์แนะนำการใช้งาน") · เพิ่มสไลด์ได้เมื่อมีฟีเจอร์ใหม่ · ปรับหน้า **วิธีใช้งาน** ให้ตรงกับฐานข้อมูล 540+ เมนู · รองรับไทย/อังกฤษ
- **v2.34.0** *(เว็บ · BETA · silent)* — 🍜 **ฐานข้อมูลอาหาร 500+ รายการ** — ขยายจาก ~250 เป็น **540 เมนู เน้นอาหารไทยเป็นหลัก** (200+ เมนู: แกง · ต้ม · ผัด · ยำ/ลาบ · ข้าว · เส้น · ปิ้งย่าง · ติ่มซำ/ของทานเล่น) รองลงมาเป็น **เครื่องดื่ม · ฟาสต์ฟู้ด · ของหวาน · ผลไม้ · อาหารทะเล** · มีระบบกันชื่อซ้ำอัตโนมัติ (ค้นหาแล้วไม่เจอรายการซ้ำ) · อัปเดตเงียบ
